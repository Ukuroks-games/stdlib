...
local function re_im2 (z1, z2)
	return type(z1) == "number" and z1 or z1.r,
...
function complex:arg ()
	-- Special cases to remove fantom imaginary or real parts:
	return self.i == 0 and (self.r > 0 and 0 or math.pi)
		or self.r == 0 and (self.i > 0 and math.pi / 2 or -math.pi / 2)
		or math.atan2(self.i, self.r)
end

function complex:exp ()
	local abs = math.exp(self.r)
	-- Special treatment for real numbers to avoid fantom imaginary parts:
	return self.i == 0 and abs or complex(abs * math.cos(self.i), abs * math.sin(self.i))
end
...
function complex:__pow (power)
	...
	if x then
		-- Euler:
		return complex(power * math.log(x)):exp()
	else
		-- de Moivre:
